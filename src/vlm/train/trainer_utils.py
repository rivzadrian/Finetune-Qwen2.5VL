...
from utils.packages import is_apollo_available, is_galore_available, is_ray_available
...
from src.vlm.model.loader import (
    load_model,
    load_tokenizer,
)
...
if TYPE_CHECKING:
    ...
    from src.vlm.hyparams.training_args import RayArguments, TrainingArguments
...
def _create_apollo_optimizer(
    ...
    if (
        len(finetuning_args.apollo_target) == 1
        and finetuning_args.apollo_target[0] == "all"
    ):
        apollo_targets = find_all_linear_modules(
            model, finetuning_args.freeze_vision_tower
        )
    else:
    ...
    for name, module in model.named_modules():
        if isinstance(module, torch.nn.Linear) and any(
            target in name for target in apollo_targets
        ):
            for param in module.parameters():
                if param.requires_grad and len(param.shape) > 1:
...
def nested_detach(
    ...
    if isinstance(tensors, (list, tuple)):
        ...
        return type(tensors)(
            {k: nested_detach(t, clone=clone) for k, t in tensors.items()}
        )
