...
from src.vlm.model.model_utils.misc import (
    find_all_linear_modules,
    find_expanded_modules,
)
from src.vlm.model.model_utils.quantization import QuantizationMethod
from src.vlm.model.model_utils.unsloth import (
    get_unsloth_peft_model,
    load_unsloth_peft_model,
)
from src.vlm.model.model_utils.visual import get_forbidden_modules, patch_target_modules
...
def _setup_lora_tuning(
    ...
    if model_args.adapter_name_or_path is not None:
        ...
        logger.info(
            "Loaded adapter(s): {}".format(",".join(model_args.adapter_name_or_path))
        )
    ...
    if (
        ...
        target_modules = patch_target_modules(
            model.config, finetuning_args, target_modules
        )
        ...
        if model_args.use_unsloth:
            ...
            if finetuning_args.pissa_init:
                if finetuning_args.pissa_iter == -1:
                    ...
                    logger.info(
                        f"Using PiSSA initialization with FSVD steps {finetuning_args.pissa_iter}."
                    )
                    peft_kwargs["init_lora_weights"] = (
                        f"pissa_niter_{finetuning_args.pissa_iter}"
                    )
...
def init_adapter(
    ...
    if finetuning_args.finetuning_type == "full":
        ...
        raise NotImplementedError(
            f"Unknown finetuning type: {finetuning_args.finetuning_type}."
        )
