...
from utils.misc import check_version
...
def configure_attn_implementation(
    ...
    if getattr(config, "model_type", None) == "gemma2" and is_trainable:
        if model_args.flash_attn == "auto" or model_args.flash_attn == "fa2":
            if is_flash_attn_2_available():
                ...
                logger.debug("FlashAttention-2 is not installed, use eager attention.")
    ...
    if model_args.flash_attn == "auto":
        ...
        if not is_torch_sdpa_available():
            logger.debug("torch>=2.1.1 is required for SDPA attention.")
            return

        requested_attn_implementation = "sdpa"
    elif model_args.flash_attn == "fa2":
        if not is_flash_attn_2_available():
            logger.debug("FlashAttention-2 is not installed.")
            return
